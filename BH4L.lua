...
local function PlayerData.new(parent, player)
...
local function PlayerData:saveLimbProperties(limb)
...
local function PlayerData:restoreLimbProperties(limb)
	local parent = self._parent
...
local function PlayerData:modifyLimbProperties(limb)
	local parent = self._parent
...
local function PlayerData:spoofSize(part)
...
local function PlayerData:setupCharacter(char)
	local parent = self._parent
...
local function PlayerData:onCharacter(char)
...
local function PlayerData:Destroy()
...
local function LimbExtender.new(userSettings)
	local self = setmetatable({
...
local function LimbExtender:Stop()
	self._running = false
	for i, pd in pairs(self._playerTable) do pd:Destroy() end
	self._playerTable = {}
end

local function LimbExtender:Destroy()
	self:Stop()
...
Themes:CreateDropdown({
    ...
    Flag = "CurrentTheme",
    Callback = function(Options)
        Window:ModifyTheme(type(Options)
    end,
})
